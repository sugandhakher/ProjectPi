...
#calculating links with no inlinks in dict1 in G2
count1=0
no_inlink = []
for link in P():
    value=c.dict1[link]
    if len(value) == 0:
        ...
        count1 += 1
# with open("G2_no_inlink.txt",'wt') as f:
...
for link in P():
    ...
    if len(value) == 0:
        Sink.append(link)
        count +=1
# with open("G2_no_outlink.txt",'wt') as f:
#     for link in Sink:
#         f.write(link +'\n')
...
def page_rank():
    ...
    print PR
    # i=0
    # with open("page-rank-file-G2",'wt') as f:
    #     for link in PR:
    #         if i>= 50:
    #             break
    #         f.write(str(link)+'\n')
